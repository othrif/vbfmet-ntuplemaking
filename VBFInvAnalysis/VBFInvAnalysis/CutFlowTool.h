...
public:
   ...
   std::map<UInt_t, TString> getCutFlowToolMap() { return cuts_map_; }
   std::vector<Double_t>     getNPassVec() { return cut_npass_; }
   Double_t                  getNPass(UInt_t index) { return cut_npass_[index]; }
   TString                   getCutName(UInt_t index) { return cuts_map_[index]; }
