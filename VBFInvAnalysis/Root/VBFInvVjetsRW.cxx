...
EL::StatusCode VBFInvVjetsRW :: fileExecute()
{
   ...
   //   const Bool_t isTruthDerivation = (MetaData->GetBranch("StreamDAOD_TRUTH1") || MetaData->GetBranch("StreamDAOD_TRUTH3"));
...
EL::StatusCode VBFInvVjetsRW::defineContainers()
{
   ...
   if (!m_determinedDerivation) {
      ...
      ANA_MSG_INFO("Is it DAOD_TRUTH? (will trigger access to dedicated truth electron and muon containers): " << isTRUTH);
   }

   const TString particles_container = "TruthParticles";
   if (!event->retrieve(particles, particles_container.Data()).isSuccess()) {    // retrieve arguments: container type, container key
   ...
   if (m_isDAODTRUTH) {
      ...
      if (!event->retrieve(muons, muons_container.Data()).isSuccess()) {    // retrieve arguments: container type, container key
         ANA_MSG_ERROR("Failed to retrieve TruthMuons container");
         return EL::StatusCode::FAILURE;
      }

      const TString electrons_container = "TruthElectrons";
      if (!event->retrieve(electrons, electrons_container.Data()).isSuccess()) {    // retrieve arguments: container type, container key
         ANA_MSG_ERROR("Failed to retrieve TruthElectrons container");
         return EL::StatusCode::FAILURE;
      }

      const TString neutrinos_container = "TruthNeutrinos";
      if (!event->retrieve(neutrinos, neutrinos_container.Data()).isSuccess()) {    // retrieve arguments: container type, container key
         ANA_MSG_ERROR("Failed to retrieve TruthNeutrinos container");
         return EL::StatusCode::FAILURE;
      }

      const TString photons_container = "TruthPhotons";
      if (!event->retrieve(photons, photons_container.Data()).isSuccess()) {    // retrieve arguments: container type, container key
         ANA_MSG_ERROR("Failed to retrieve TruthPhotons container");
         return EL::StatusCode::FAILURE;
      }

      const TString boson_container = "TruthBoson";
      if (!event->retrieve(bosons, boson_container.Data()).isSuccess()) {    // retrieve arguments: container type, container key
   ...
   for (auto thisParticle : *m_particles.first) {
      ...
      if (thisParticle->status() == 3) {
         if (abs(thisParticle->pdgId()) == 11 || abs(thisParticle->pdgId()) == 13 || abs(thisParticle->pdgId()) == 15)
            m_n_partonLeptons ++;
         else if (abs(thisParticle->pdgId()) == 12 || abs(thisParticle->pdgId()) == 14 || abs(thisParticle->pdgId()) == 16)
            m_n_partonNeutrinos ++;
         if (abs(thisParticle->pdgId()) == 22)
            m_n_partonPhotons ++;
      }
   }
...
EL::StatusCode VBFInvVjetsRW::fillParticle(xAOD::TruthParticle* thisParticle)
{
   ...
   m_truth_mc_p4_dressed.SetPtEtaPhiE(thisParticle->auxdata< float >("pt_dressed"),
                                      thisParticle->auxdata< float >("eta_dressed"),
                                      thisParticle->auxdata< float >("phi_dressed"),
                                      thisParticle->auxdata< float >("e_dressed"));
