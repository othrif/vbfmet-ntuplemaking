...
double Analysis::outRnSmear::getPSweight(asg::AnaToolHandle<ST::ISUSYObjDef_xAODTool> susytools_handle,
                                         xAOD::TEvent *&event, int RunNumber, bool debug)
{

   if (debug)
      Info("getPSweight()",
           "---------------------------- Jet trigger pre-scales for Rebalance and Smear ----------------------------");
   ...
   for (const std::string &sjt : single_jet_triggers) {
      ...
      if (debug) std::cout << "Trigger, fired: " << sjt << ", " << susytools_handle->IsTrigPassed(sjt) << std::endl;
   }
   ...
   if (i_highest_fire == -1 || i_highest_threshold == -1) {
      ...
      if (debug) std::cout << "Combined pre-scale = " << PSweight << std::endl;
   }
