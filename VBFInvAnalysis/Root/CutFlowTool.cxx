...
void Analysis::CutFlowTool::print()
{
   ...
   for (it = cuts_map_.begin(); it != cuts_map_.end(); it++) {
    ...
    if (cut_npass_.at(index_minus_one) != 0 && cut_npass_.at(0) != 0) {
     Info("CutFlowTool", "| %20s | %.2lf\t| %.2lf\t | %.2lf |", ((*it).second).Data(), cut_npass_.at(index), (double)cut_npass_.at(index) / (double)cut_npass_.at(1), (double)cut_npass_.at(index) / (double)cut_npass_.at(index_minus_one));
 } else {
...
void Analysis::CutFlowTool::printUnw()
{
   ...
   for (it = cuts_map_.begin(); it != cuts_map_.end(); it++) {
    ...
    if (cut_npass_unw_.at(index_minus_one) != 0 && cut_npass_unw_.at(0) != 0) {
     Info("CutFlowTool", "| %20s | %.2lf\t| %.2lf\t | %.2lf |", ((*it).second).Data(), cut_npass_unw_.at(index), (double)cut_npass_unw_.at(index) / (double)cut_npass_unw_.at(1), (double)cut_npass_unw_.at(index) / (double)cut_npass_unw_.at(index_minus_one));
 } else {
     std::cout << (*it).second << ": " << cut_npass_unw_.at(index) << ", efficiencies could not be computed since the denominators are 0" << std::endl;
 }
}
